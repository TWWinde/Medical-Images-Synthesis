...
class CocoStuff164k(_BaseDataset):
    ...
    def __init__(self,files_list,**kwargs):
        self.files_val_for_sis = files_list
    ...
    def _set_files(self):
        ...
        '''if self.split in ["train2017", "val2017"]:
            ...
            self.files = file_list'''
        '''if self.split in ["val_label"]:
            file_list = sorted(glob(osp.join(self.root, self.split, "*.png")))
            assert len(file_list) > 0, "{} has no image".format(
                osp.join(self.root, self.split)
            )
            file_list = [f.split("/")[-1].replace(".png", "") for f in file_list]
            self.files = file_list
        else:
            raise ValueError("Invalid split name: {}".format(self.split))'''
        self.files = self.files_val_for_sis
    ...
    def _load_data(self, index):
        ...
        return image_id, image, label'''
        image_id = index
        image_path = self.files[image_id]['fpath_img']
        label_path = self.files[image_id]['fpath_segm']
        # Load an image and label
        image = cv2.imread(image_path, cv2.IMREAD_COLOR).astype(np.float32)
    ...
    def __getitem__(self, index):
        ...
        label = label - 1
        label[label == -1] = 255
        return image_id, image.astype(np.float32), label

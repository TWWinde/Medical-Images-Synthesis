...
#--- create models ---#
model = models.Unpaired_model_gc(opt)
model = models.put_on_multi_gpus(model, opt)

#--- create optimizers ---#
optimizerG = torch.optim.Adam(model.module.netG.parameters(), lr=opt.lr_g, betas=(opt.beta1, opt.beta2))
optimizerDu = torch.optim.Adam(list(model.module.netDu.parameters()) + list(model.module.netDu_gc.parameters()), lr=5*opt.lr_d, betas=(opt.beta1, opt.beta2))
...
for epoch in range(start_epoch, opt.num_epochs):
    for i, data_i in enumerate(dataloader):
        ...
        loss_G, losses_G_list = model(image, label, "losses_G_gc", losses_computer)
        ...
        loss_Du, losses_Du_list = model(image, label, "losses_Du_gc", losses_computer)

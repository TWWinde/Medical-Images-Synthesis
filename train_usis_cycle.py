...
#--- create optimizers ---#
optimizerG = torch.optim.Adam(itertools.chain(model.module.netG.parameters(), model.module.netD.parameters()), lr=opt.lr_g, betas=(opt.beta1, opt.beta2))
optimizerDu = torch.optim.Adam(itertools.chain(model.module.netDu_image.parameters(), model.module.netDu_label.parameters()), lr=5*opt.lr_d, betas=(opt.beta1, opt.beta2))
...
for epoch in range(start_epoch, opt.num_epochs):
    for i, data_i in enumerate(dataloader):
        ...
        loss_G, losses_G_list = model(image, label, "losses_G_cycle", losses_computer)
        loss_G, losses_G_list = loss_G.mean(), [loss.mean() if loss is not None else None for loss in losses_G_list]
        loss_G.backward()
        optimizerG.step()

        p_loss_G, p_losses_G_list = torch.zeros((1)), [torch.zeros((1))]
        losses_D_list = [None, None, None]

        #--- unconditional discriminator update ---#
        model.module.netDu_image.zero_grad()
        model.module.netDu_label.zero_grad()
        loss_Du, losses_Du_list = model(image, label, "losses_Du_cycle", losses_computer)
